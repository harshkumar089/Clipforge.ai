...
export const RegisterPage: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const res = await api.googleSignIn('ashme@gmail.com', 'Ashme');
      if (res.success && res.token && res.user) {
        login(res.token, res.user);
        showToast(`Welcome to ClipForge, ${res.user.name || res.user.email}!`, 'success');
        navigate('/dashboard');
      }
    } catch (err: any) {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 w-full">
        <div className="glass-panel py-8 px-5 sm:px-10 rounded-2xl border border-purple-100 dark:border-purple-800/60 bg-white dark:bg-[#120a26] shadow-xl shadow-purple-500/5">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 mb-4 rounded-xl border border-slate-200 dark:border-purple-800/80 bg-white dark:bg-[#1a1033] hover:bg-slate-50 dark:hover:bg-[#221544] text-slate-700 dark:text-purple-100 font-medium text-sm shadow-sm flex items-center justify-center gap-3 transition-all hover:scale-[1.01] cursor-pointer disabled:opacity-60"
          >
            {googleLoading ? (
