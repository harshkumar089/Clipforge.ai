...
import { Scissors, ArrowRight, Loader2, Eye, EyeOff, UserPlus, X } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      const messages: Record<string, string> = {
        oauth_not_configured: 'Google Sign-In is not configured with Google Cloud yet. Enter your Google email below.',
        ...
        missing_google_email: 'Please provide a valid Google email address.',
      };
      showToast(messages[errorParam] || `Sign-in error: ${errorParam}`, 'error');
    }
  }, [searchParams, showToast]);

  // Handle standard credentials login (Email + Password for existing user accounts)
  ...
  const handleGoogleSignInClick = async () => {
    try {
      ...
      // If live Google Cloud OAuth is configured with credentials, redirect to Google OAuth
      if (status.hasLiveGoogleAuth) {
        window.location.href = '/api/auth/google';
        return;
      }

      // If no live keys in .env, open the Google Account prompt modal to sign in with their personal Google ID
  ...
  // Complete Google Sign-In with user's personal Google email & name
  const handleModalGoogleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!modalGoogleEmail || !modalGooglePassword) {
      showToast('Please enter both your Google email and password.', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.success && res.token && res.user) {
        ...
        showToast(`Signed in successfully with Google (${res.user.email})!`, 'success');
        navigate('/dashboard');
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#faf9fe] dark:bg-[#090514] flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center z-10">
        ...
        <p className="mt-2 text-xs text-slate-500 dark:text-purple-300/70">
          Enter your login credentials or continue with your Google account.
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 w-full">
        <div className="glass-panel py-8 px-5 sm:px-10 rounded-2xl border border-purple-100 dark:border-purple-800/60 bg-white dark:bg-[#120a26] shadow-xl shadow-purple-500/5">
          ...
          {/* 1. Direct Sign in with Google Button */}
          <button
            type="button"
          ...
          {/* 2. Credentials Form for Existing Accounts */}
          <form onSubmit={handleCredentialsSubmit} className="space-y-4">
            <div>
              <label htmlFor="login-email" className="block text-xs font-semibold text-slate-700 dark:text-purple-200">
                Email Address
              </label>
              <input
                id="login-email"
            ...
            <div>
              <label htmlFor="login-password" className="block text-xs font-semibold text-slate-700 dark:text-purple-200">
                Password
              </label>
      ...
      {/* Personal Google ID Sign-In Modal (when live Google Cloud keys are not set in .env) */}
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white dark:bg-[#160d30] border border-purple-200 dark:border-purple-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <form onSubmit={handleModalGoogleSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-purple-200">Your Name (Optional)</label>
                <input
                  type="text"
              ...
              >
                {googleLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Continue to Dashboard'}
              </button>
            </form>
          </div>
        </div>
